<?php

require_once __DIR__ . '/database.php';
require_once __DIR__ . '/../core/Comando.php';
require_once __DIR__ . '/../core/Player.php';
require_once __DIR__ . '/../core/Mundo.php';
require_once __DIR__ . '/../core/MundoMapa.php';
require_once __DIR__ . '/../core/MundoNPC.php';
require_once __DIR__ . '/../core/MundoItem.php';
require_once __DIR__ . '/../core/MundoDebug.php';
require_once __DIR__ . '/../core/Combate.php';
require_once __DIR__ . '/../core/DesfechoCombate.php';
require_once __DIR__ . '/../core/Evento.php';
require_once __DIR__ . '/../core/EventoService.php';
require_once __DIR__ . '/../controllers/PlayerController.php';
require_once __DIR__ . '/../controllers/NPCController.php';
require_once __DIR__ . '/../controllers/MagiaController.php';
require_once __DIR__ . '/../controllers/EventoController.php';
require_once __DIR__ . '/../controllers/CombateController.php';
